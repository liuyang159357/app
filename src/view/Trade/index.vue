  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="item in userAddress" :key="item.id">
        <span :class="{ username, selected: item.isDefault == 1 }">{{ item.consignee }}</span>
        <p @click="changeDefult(item)">
          <span class="s1">{{ item.fullAddress }}</span>
          <span claass="s2">{{ item.phoneNum }}</span>
          <span class="s3" v-show="item.isDeefult == 1">默认地址</span>
        </p>
      </div>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ shopAddress&& shopAddress.fullAddress}}</span>
        收货人：<span>{{ shopAddress&& shopAddress.consignee}}</span>
        <span>{{ shopAddress&& shopAddd.phoneNum}}</span>
...
export default {
  ...
  computed: {
    ...
    shopAddress() {
      return this.userAddress.find(i => i.isDefault == 1)
    },
    detailArrayList() {
